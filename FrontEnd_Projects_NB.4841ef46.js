...
},{"../modules/Game.class":"dhlFI","@parcel/transformer-js/src/esmodule-helpers.js":"jnFvT"}],"dhlFI":[function(require,module,exports,__globalThis) {
'use strict';
/**
 ...
 */ class Game {
   ...
   */ constructor(initialState){
    ...
    moveRight() {
        ...
        let gained = 0;
        this.board = this.board.map((row)=>{
            const [slid, rowScore] = this._slideLeft([
                ...row
            ].reverse());
            gained += rowScore;
            return slid.reverse();
   ...
   */ restart() {
    ...
    _slideLeft(row) {
        ...
        let scoreGained = 0;
        let i = 0;
        while(i < tiles.length)if (i + 1 < tiles.length && tiles[i] === tiles[i + 1]) {
    ...
    _getEmptyCells() {
        const cells = [];
        for(let r = 0; r < 4; r++){
            for(let c = 0; c < 4; c++)if (this.board[r][c] === 0) cells.push([
                r,
                c
            ]);
        }
    ...
    _updateStatus() {
        ...
        if (!this._hasMovesAvailable()) this.status = 'lose';
    }
    _hasMovesAvailable() {
...
//# sourceMappingURL=FrontEnd_Projects_NB.4841ef46.js.map
